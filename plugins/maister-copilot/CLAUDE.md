...
UI mockups are generated by the shared **`mockup-studio`** skill, used by both the development workflow (Phase 4) and the product-design workflow (Phase 7), and invocable standalone (`/mockup-studio`). It runs in the main agent context, so it can drive a live visual-companion server, a browser preview, and interactive refinement gates.
...
- Standalone — `/mockup-studio "<screen or feature>"`
...
```
.maister/
├── config.yml                    # Project configuration (optional; scaffolded by /init)
...
An optional project-level config file holds defaults that apply to every workflow. `/init` scaffolds it with documented defaults; orchestrators read it at initialization and fall back to defaults when it is absent (so existing projects are unaffected).
...
|---------|-------|---------|
| `/init` | `/init [--standards-from=PATH]` | Initialize framework with project analysis and smart defaults for docs/standards. Optionally copy standards from another project's `.maister/docs/standards/` instead of built-in defaults. |
...
> **Note**: These are all skills (not commands). `/init`, `/standards-update`, and `/standards-discover` invoke their respective skills which delegate file operations to the internal `docs-manager` skill.
...
|---------|-------|----------------|
| `/development` | `[desc] [--e2e] [--user-docs] [--research=PATH] [--sequential]` (new) / `[task-path] [--from=PHASE] [--reset-attempts] [--sequential]` (resume) | `.maister/tasks/development/` |
| `/performance` | `[desc] [--sequential]` (new) / `[task-path] [--from=PHASE] [--sequential]` (resume) | `.maister/tasks/performance/` |
| `/migration` | `[desc] [--type=TYPE] [--sequential]` (new) / `[task-path] [--from=PHASE] [--sequential]` (resume) | `.maister/tasks/migrations/` |
