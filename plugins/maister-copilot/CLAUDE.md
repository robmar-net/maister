...
UI mockups are generated by the shared **`mockup-studio`** skill, used by both the development workflow (Phase 4) and the product-design workflow (Phase 7), and invocable standalone (`/maister-copilot:mockup-studio`). It runs in the main agent context, so it can drive a live visual-companion server, a browser preview, and interactive refinement gates.

**Two formats** (controlled by `mockup_format` in `.maister/config.yml`, default `html`):
- **HTML** (default): rendered HTML/CSS via the zero-dependency Node visual companion (browser preview, saved as `.html` files). Auto-falls back to ASCII when Node.js is unavailable.
- **ASCII**: annotated terminal diagrams via the `ascii-mockup-generator` agent (no Node/browser). Set `mockup_format: ascii` (≡ product-design's `--no-visual`).

**Before generating**, mockup-studio runs **design-resource discovery** (3 tiers — project standards, codebase design system, available design skills/MCP tools) and binds mockups to the project's real tokens, components, and conventions. See `skills/mockup-studio/references/design-resource-discovery.md`.

**When Used**:
- Development Phase 4 — auto-triggered when `task_characteristics.ui_heavy` is true (single pass; the Phase 4 gate handles approve/revise)
- Product-design Phase 7 — when `is_ui_focused` (full interactive refinement loop)
- Standalone — `/maister-copilot:mockup-studio "<screen or feature>"`

**Output**: HTML → `<output_subdir>/*.html` (`analysis/design-context/mockups/` in development, `analysis/mockups/` in product-design); ASCII → `analysis/design-context/ascii/ui-mockups.md`. Both append stable screen/component IDs to `analysis/design-context/INDEX.md` (in development) so the planner can attach `Visual References`. A `design-resources.md` inventory is written alongside.

**Benefits**:
- Visualize layout before implementation (rendered HTML or ASCII)
- Bind to the project's existing design language (standards, tokens, components)
- Identify reusable components early
- Single reusable engine shared across workflows
- No external design tools needed (zero-dependency server)
...
```
.maister/
├── config.yml                    # Project configuration (optional; scaffolded by /maister-copilot:init)
...
An optional project-level config file holds defaults that apply to every workflow. `/maister-copilot:init` scaffolds it with documented defaults; orchestrators read it at initialization and fall back to defaults when it is absent (so existing projects are unaffected).
...
- **`mockup_format`** (default `html`): how UI mockups are rendered when a workflow generates them (development Phase 4, product-design Phase 7, standalone `/maister-copilot:mockup-studio`). `html` → the `mockup-studio` visual companion (browser preview, `.html` files); `ascii` → the `ascii-mockup-generator` agent (no Node/browser). Auto-falls back to `ascii` when Node.js is unavailable. Independent of `html_output` (mockups are design deliverables, not report companions). Read once at init, seeded into `orchestrator.options.mockup_format`.
...
│   │   ├── mockups/             # HTML mockups (Phase 4 mockup-studio, product-design, or inline refs) + PNG/screenshots
...
| `mockup-studio` | Shared UI-mockup engine: runs design-resource discovery, then renders HTML (visual companion server + browser) or ASCII (via `ascii-mockup-generator`). User-invocable standalone AND invoked by development (Phase 4) and product-design (Phase 7) via the Skill tool. Owns the moved visual-companion server (`skills/mockup-studio/server/`). | `skills/mockup-studio/SKILL.md` |
...
|---------|-------|---------|
| `/maister-copilot:init` | `/maister-copilot:init [--standards-from=PATH]` | Initialize framework with project analysis and smart defaults for docs/standards. Optionally copy standards from another project's `.maister/docs/standards/` instead of built-in defaults. |
| `/maister-copilot:standards-update` | `/maister-copilot:standards-update [description] [--from=PATH]` | Update/create standards from conversation context, or sync from another project |
| `/maister-copilot:standards-discover` | `/maister-copilot:standards-discover [--scope=SCOPE]` | Discover standards from config files and code patterns |

> **Note**: These are all skills (not commands). `/maister-copilot:init`, `/maister-copilot:standards-update`, and `/maister-copilot:standards-discover` invoke their respective skills which delegate file operations to the internal `docs-manager` skill.
...
|---------|-------|----------------|
| `/maister-copilot:development` | `[desc] [--e2e] [--user-docs] [--research=PATH] [--sequential]` (new) / `[task-path] [--from=PHASE] [--reset-attempts] [--sequential]` (resume) | `.maister/tasks/development/` |
| `/maister-copilot:performance` | `[desc] [--sequential]` (new) / `[task-path] [--from=PHASE] [--sequential]` (resume) | `.maister/tasks/performance/` |
| `/maister-copilot:migration` | `[desc] [--type=TYPE] [--sequential]` (new) / `[task-path] [--from=PHASE] [--sequential]` (resume) | `.maister/tasks/migrations/` |
| `/maister-copilot:research` | `[question] [--type=TYPE] [--brainstorm] [--no-brainstorm] [--design] [--no-design]` (new) / `[task-path] [--from=PHASE]` (resume) | `.maister/tasks/research/` |
| `/maister-copilot:product-design` | `[desc] [--research=PATH] [--no-visual]` (new) / `[task-path] [--from=PHASE]` (resume) | `.maister/tasks/product-design/` |
...
| Command | Usage | Purpose |
|---------|-------|---------|
| `/maister-copilot:reviews-code` | `[path] [--scope=SCOPE]` | Automated code quality, security, performance analysis |
| `/maister-copilot:reviews-pragmatic` | `[path]` | Detect over-engineering, ensure code matches project scale |
| `/maister-copilot:reviews-spec-audit` | `[spec-path]` | Independent spec audit for completeness and clarity |
| `/maister-copilot:reviews-reality-check` | `[task-path]` | Validate work actually solves the problem |
| `/maister-copilot:reviews-production-readiness` | `[path] [--target=ENV]` | Pre-deployment verification with GO/NO-GO recommendation |

### Quick Commands

| Command | Usage | Purpose |
|---------|-------|---------|
| `/maister-copilot:quick-plan` | `[task description]` | Enter planning mode with standards awareness from INDEX.md |
| `/maister-copilot:quick-dev` | `[task description]` | Implement directly with standards awareness (no planning) |
| `/maister-copilot:quick-bugfix` | `[bug description]` | Quick bug fix with TDD red/green gates and complexity escalation |
...
|-------|---------|------------|---------|
| `project-analyzer` | Deep codebase analysis for tech stack, architecture, conventions | `/maister-copilot:init` | `agents/project-analyzer.md` |
...
| `task-classifier` | Classifies task descriptions into workflow types with confidence scoring | `/maister-copilot:work` command | `agents/task-classifier.md` |
...
The plugin includes hooks that fire at specific GitHub Copilot CLI lifecycle events.
...
**See**: `hooks/hooks.json` for hook configuration (auto-discovered by GitHub Copilot CLI).
