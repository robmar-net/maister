...
validate:
	...
	@! grep -rnE '(^|[^/`])maister-copilot:(development|migration|quick-plan)([^a-z-]|$$)' plugins/maister-copilot --include='*.md' || (echo "FAIL: skill referenced as maister-copilot:<skill> above; a MODEL-facing skill reference must be bare (skill(skill: \"development\") / <available_skills>). Only the USER-facing SLASH form carries the prefix — /maister-copilot:development (#132)" && exit 1)
	...
	@echo "Checking no generated SQL instruction carries Claude's todos status enum 'completed' (WS5.20, #114)..."
	@! grep -rnE "status[[:space:]]*=[[:space:]]*['\"]completed['\"]" plugins/maister-copilot/ 2>/dev/null || (echo "FAIL: SQL-shaped todos write with Claude's 'completed' enum above; Copilot's pre-seeded todos table uses status in (pending,in_progress,done), so this instruction would make the model write a value the schema does not define. Fix the transform in build.sh (task-tool -> SQL session store) - the plugin root CLAUDE.md is NOT a runtime channel and cannot correct it" && exit 1)
	@echo "Checking the generated SessionStart reminder emits the FLAT additionalContext envelope (WS5.21, #113)..."
	@bash plugins/maister-copilot/hooks/skill-invocation-reminder.sh | grep -qE '^[[:space:]]{0,2}"additionalContext":' || (echo "FAIL: generated hooks/skill-invocation-reminder.sh does not emit a TOP-LEVEL \"additionalContext\" key; Copilot's sessionStart hook reads only the flat shape, so any other envelope makes the reminder a silent no-op (verified live on 1.0.82, #113). Fix platforms/copilot-cli/hooks-overrides/skill-invocation-reminder.sh, then make build" && exit 1)
	@! bash plugins/maister-copilot/hooks/skill-invocation-reminder.sh | grep -q 'hookSpecificOutput' || (echo "FAIL: generated hooks/skill-invocation-reminder.sh wraps its output in Claude's hookSpecificOutput; Copilot SILENTLY DROPS that wrapper for sessionStart (the CLI still logs it, so a debug-log grep will not catch this - #113). Emit additionalContext at the top level in platforms/copilot-cli/hooks-overrides/, then make build" && exit 1)
	@echo "Checking no bare slash-command form survives into the generated variant (WS5.22, #132)..."
	@! grep -rnE '(^|`)/(development|research|performance|migration|product-design|quick-plan|quick-dev|quick-bugfix|work|init|standards-update|standards-discover|mockup-studio|reviews-[a-z-]+)([^a-z0-9:-]|$$)' plugins/maister-copilot/ 2>/dev/null || (echo "FAIL: bare slash-command form above in the generated variant. Copilot's slash completion offers only the plugin-id-prefixed form (/maister-copilot:<name>); a bare /<name> is not offered and does not run, so this instruction hands the user a dead command. build.sh step 4d prefixes every slash token - regenerate with make build" && exit 1)
	@echo "Checking CLAUDE.md still imports AGENTS.md, so the working agreement reaches agents (WS5.23, #134)..."
	@grep -q '@AGENTS.md' CLAUDE.md || (echo "FAIL: CLAUDE.md no longer imports AGENTS.md. Claude Code auto-loads ONLY the repo-root CLAUDE.md, so without that import every binding rule in AGENTS.md - never push upstream, +fork.N versioning, the merge tripwires, one-ticket-one-worktree - silently stops reaching agent sessions. Nothing errors when instructions are absent, which is why this is a guard and not a convention. An upstream merge overwriting CLAUDE.md is the likely cause: restore the @AGENTS.md line at the top of CLAUDE.md" && exit 1)
